...
const components: MDXComponents = {
  ...
  // リスト
  ul: ({ children }) => (
    <ul className="list-disc list-inside space-y-3 mb-6 text-brown-700 ml-4 marker:text-forest-600">
      {children}
    </ul>
  ),
  ol: ({ children }) => (
  ...
  tbody: ({ children }) => <tbody className="bg-white divide-y divide-cream-200">{children}</tbody>,
